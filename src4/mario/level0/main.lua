...
function generateLevel()
    ...
    for x = 1, mapWidth do
        ...
        spawnPillar = math.random(5) == 1

        -- if the column is pillar
        if spawnPillar then
            for height = 5, 6 do
                tiles[height][x] = {
                    id = GROUND,
                    topper = height == 5 and true or false,
                }
            end
        end
        
        -- always generate ground
        for y = 7, mapHeight do
            tiles[y][x] = {
                id = GROUND,
                topper = (not spawnPillar and y == 7) and true or false,
            }
        end

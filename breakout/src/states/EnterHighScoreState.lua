...
function EnterHighScoreState:render()
    ...
    -- enter player name (3 character)
    love.graphics.setFont(gFonts['large'])
    
    if highlightedChar == 1 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.printf(string.char(chars[1]), VIRTUAL_WIDTH / 2 - 28, VIRTUAL_HEIGHT / 2, 27, 'left')
    love.graphics.setColor(1, 1, 1, 1)

    if highlightedChar == 2 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.printf(string.char(chars[2]), VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2, 27, 'left')
    love.graphics.setColor(1, 1, 1, 1)

    if highlightedChar == 3 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.printf(string.char(chars[3]), VIRTUAL_WIDTH / 2 + 28, VIRTUAL_HEIGHT / 2, 27, 'left')
    love.graphics.setColor(1, 1, 1, 1)
...
function EnterHighScoreState:update(dt)
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        local new_name = string.char(chars[1]) .. string.char(chars[2]) .. string.char(chars[3])
        ...
        self.highScores[self.scoreIndex].name = new_name

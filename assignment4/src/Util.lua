...
--[[
    Generate the flag from 'graphics/flags.png'
]]
function GenerateFlagQuads(atlas)
    ...
    -- generate the poles
    for x = 1, 6 do
        table.insert(quads, love.graphics.newQuad((x - 1) * 16, 0, 16, 48, atlas:getDimensions()))
    end

    -- generate the flag
    for y = 1, 4 do
        table.insert(quads, love.graphics.newQuad(96, (y - 1) * 16, 48, 16, atlas:getDimensions()))
    end

...
function PlayState:enter(params)
    if params then
        self.player.score = params.player.score
        local newWidth = params.width + 20
        print(newWidth)

        self.level = LevelMaker.generate(newWidth, 10)
        self.tileMap = self.level.tileMap

        -- reinitialize the level and tile map if the first column spawn chasm
        while self.tileMap.tiles[7][1].id == TILE_ID_EMPTY do
            self.level = LevelMaker.generate(newWidth, 10)
            self.tileMap = self.level.tileMap
        end

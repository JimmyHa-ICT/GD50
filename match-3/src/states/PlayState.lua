    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                self:swapTile(newTile, self.highlightedTile)
                ...
                :finish(function()
                    -- if there are no match, swap back two tile
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            -- gain score if tile in match is star, triangle, etc.
            for k, tile in pairs(match) do
                if tile.shiny then
                    self.score = self.score + 500

                    -- extend the timer 10 seconds
            ...
            if notShiny then
                ...
                for k, tile in pairs(match) do
                    self.score = self.score + (tile.variety - 1) * 10
                end

                -- extend the timer 1 second for each tile in match
...
function PlayState:swapTile(tile1, tile2)

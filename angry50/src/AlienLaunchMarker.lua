...
function AlienLaunchMarker:init(world)
    ...
    -- flag set when the alien can split 3 or not
    self.splitable = true
end
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        -- if space pressed, split alien into three aliens
        if love.keyboard.wasPressed('space') and self.splitable then
            alien = self.aliens[1]
            local xPos, yPos = alien.body:getPosition()
            local xVel, yVel = alien.body:getLinearVelocity()

            -- spawn new alien in the world, passing in user data of player
            alien1 = Alien(self.world, 'round', xPos, yPos, 'Player')
            alien2 = Alien(self.world, 'round', xPos, yPos, 'Player')
            table.insert(self.aliens, alien1)
            table.insert(self.aliens, alien2)

            -- apply the difference between current X,Y and base X,Y as launch vector impulse
            alien1.body:setLinearVelocity(xVel - 0.2 * yVel, yVel + 0.2 * xVel)
            alien2.body:setLinearVelocity(xVel + 0.2 * yVel, yVel - 0.2 * xVel)
            ...
            -- set the splitable flag to false
